...
def vehicle_control(vehicle, max_speed_mps = 10):
    global manual_mode, previous_error, integral, visual_error
...
def main():
    ...
    weather = carla.WeatherParameters(
        ...
        precipitation=10.0,             # 0–100 → Lluvia (0 = sin lluvia, 100 = lluvia intensa)
        precipitation_deposits=50.0,    # 0–100 → Charcos en el suelo
        wind_intensity=50.0,            # 0–100 → Viento
        sun_azimuth_angle=200.0,         # 0–360° → Dirección del sol (horizontal)
        sun_altitude_angle=90.0,        # -90 a 90° → Altura del sol (90 = mediodía)
        fog_density=20.0,               # 0–100 → Grosor de la niebla
