...
def subsample_by_beams(points, semantic_tags, channels=64, step=2):
    """
    Submuestrea eliminando canales completos (haces verticales).
    Ej: step=2 → conserva 1 de cada 2 haces.
    """
    N = len(points)
    points_per_channel = N // channels

    mask = np.zeros(N, dtype=bool)

    for c in range(0, channels, step):
        start = c * points_per_channel
        end   = (c + 1) * points_per_channel
        mask[start:end] = True

    return points[mask], semantic_tags[mask]
...
def lidar_callback(lidar_data, downsampled_point_cloud, frame,lidar, lidar_range, hfov, upper_fov, lower_fov , noise_std=0.1, attenuation_coefficient=0.1, output_dir = 'dataset/lidar'):
    ...
    points, semantic_tags = subsample_by_beams(points, semantic_tags, step=4) # 32 canales
