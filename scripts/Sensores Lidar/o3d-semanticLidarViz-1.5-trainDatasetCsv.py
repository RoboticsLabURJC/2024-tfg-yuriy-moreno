...
CSV_CONTROL_PATH = "dataset/controlFinal3.csv"
#========================================
...
def segmentation_callback(image, display_surface, frame):
    ...
    if M["m00"] != 0:
        ...
        raw_error = cX - x_center  # en píxeles
        visual_error = raw_error / x_center  # ahora en [-1, 1]
...
manual_throttle = 0.0
manual_steer = 0.0# Control del vehículo manual o automátic
...
def vehicle_control(vehicle, max_speed_mps = 10):
    ...
    if manual_mode:
        ...
        last_v = manual_throttle              # guardas velocidad real
    ...
    # Mejora del control PID
    # Ganancias para error en [-1,1]
    Kp = 0.6
    Ki = 0.02
    Kd = 0.1

    # límites integral y suavizado derivada
    I_MAX = 5.0          # límite al término integral
    ALPHA_D = 0.7        # 0..1, cuanto más alto más suave la derivada
    ...
    derivative = ALPHA_D * derivative + (1.0 - ALPHA_D) * raw_derivative
    ...
    v_max, v_min = 8.0, 2.0          # m/s
    K_v = 0.5                        # qué tan fuerte corriges la velocidad

    # 1) target_speed depende de lo que giras
    target_speed = v_max - 5.0 * abs(steer_cmd)   # más giro → menos velocidad
    target_speed = np.clip(target_speed, v_min, v_max)

    # 2) control proporcional sobre la velocidad
    speed_error = target_speed - speed
    throttle = K_v * speed_error

    # saturar throttle a [0,1]
    ...
    last_v = control.throttle   # valores tipo 4.0, 6.5, etc.
    last_w = steer_cmd      # [-1, 1]
