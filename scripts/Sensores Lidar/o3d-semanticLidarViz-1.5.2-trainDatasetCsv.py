...
# NUEVO: para dataset de control
last_v = 0.0   # último mando de velocidad (escala "alta", tipo 4–8)
last_w = 0.0   # último mando de giro (-1 a 1)
CSV_CONTROL_PATH = "dataset/control.csv"
#========================================
...
#========================================
#           Modelo ONNX PilotNet
#========================================

ONNX_MODEL_PATH = "carlaFinal2.onnx"   # ruta a tu modelo

ort_session = ort.InferenceSession(ONNX_MODEL_PATH, providers=["CUDAExecutionProvider", "CPUExecutionProvider"])
input_name = ort_session.get_inputs()[0].name
output_name = ort_session.get_outputs()[0].name

# guardaremos aquí la última imagen de segmentación
last_seg_image = None

def preprocess_for_model(img):
    """
    img: numpy (H, W, 3) en RGB [0..255]
    salida: (1, 3, 66, 200) float32 normalizado como en entrenamiento
    """
    # si viene en BGR, conviértela a RGB
    ...
    img = img[np.newaxis, :, :, :]      # batch=1
    return img.astype(np.float32)

def model_predict_controls(img):
    """
    Devuelve (v_cmd, w_cmd) a partir de una imagen de segmentación.
    """
    model_input = preprocess_for_model(img)
    output = ort_session.run([output_name], {input_name: model_input})
    v_cmd, w_cmd = output[0][0]   # (1,2)
...
def segmentation_callback(image, display_surface, frame):
    global visual_error
    global last_v, last_w, CSV_CONTROL_PATH  # NUEVO
    global last_seg_image # para modelo ONNX
    global frames_since_manual
    ...
    last_seg_image = seg_array.copy()  # Guardar la última imagen de segmentación para el modelo ONNX
    ...
    if frame % 1 == 0:
        ...
        # NUEVO: escribir una fila en el CSV de control
        # Ruta relativa (como en tu ejemplo de dataset)
        rel_path = os.path.join("segmentation", f"segmentation_{frame:04d}.png")
        
        
        if manual_mode:
            frames_since_manual = 0  # reset

        else:
            frames_since_manual += 1

            if frames_since_manual >= 3: 
            # estamos en modo PID -> SÍ grabar
                with open(CSV_CONTROL_PATH, "a") as f:
...
def vehicle_control(vehicle, max_speed_mps = 10):
    ...
    if manual_mode:
        global manual_throttle, manual_steer

        # Parámetros de suavizado
        accel_step = 0.5 * dt     # cuánto sube el gas por segundo manteniendo W
        brake_step = 0.5 * dt     # cuánto baja cuando sueltas
        steer_step = 0.5 * dt     # velocidad de giro
        steer_decay = 1.0 * dt    # centrado del volante

        # Acelerador / freno
        if keys[pygame.K_w]:
            if speed < max_speed_mps:
                manual_throttle = min(1.0, manual_throttle + accel_step)
            else:
                manual_throttle = 0  # mantener velocidad máxima
        elif keys[pygame.K_s]:
            manual_throttle = max(0.0, manual_throttle - brake_step)
        else:
            # si no pulsas nada, el gas se va relajando poco a poco
            manual_throttle = max(0.0, manual_throttle - brake_step * 0.5)

        # Dirección
        if keys[pygame.K_a]:
            manual_steer = max(-1.0, manual_steer - steer_step)
        elif keys[pygame.K_d]:
            manual_steer = min(1.0, manual_steer + steer_step)
        else:
            # volante vuelve hacia el centro solo
            if manual_steer > 0:
                manual_steer = max(0.0, manual_steer - steer_decay)
            elif manual_steer < 0:
                manual_steer = min(0.0, manual_steer + steer_decay)
        ...
        last_v = speed              # guardas velocidad real
        last_w = manual_steer       # y dirección suave
        return
    # Si no, estamos en modo automático
    # --- Modo automático: usar modelo ONNX si hay imagen ---
    from math import copysign
    global last_seg_image

    if last_seg_image is not None:
        # Obtener v, w del modelo
        v_cmd, w_cmd = model_predict_controls(last_seg_image)

        # Aseguramos rangos
        v_cmd = float(np.clip(v_cmd, 0.0, 1.0))      # throttle “lógico” [0,1]
        w_cmd = float(np.clip(w_cmd, -1.0, 1.0))     # giro [-1,1]

        # 2) Convertir v_cmd a velocidad objetivo [v_min, v_max]
        v_min, v_max = 4.0, 8.0   # m/s (ajustable)
        target_speed = v_min + v_cmd * (v_max - v_min)

        # 3) Lazo proporcional de velocidad
        K_v = 0.5
        speed_error = target_speed - speed
        throttle = K_v * speed_error

        # 4) Saturaciones y seguridad
        throttle = float(np.clip(throttle, 0.0, 1.0))

        # Seguridad extra: si se pasa MUCHO de v_max, frenamos un poco
        if speed > v_max + 1.0:
            throttle = 0.0
            brake = 0.0
        else:
        ...
        # w_cmd se supone en [-1,1], lo saturamos por si acaso
        ...
        # Fallback: si aún no tenemos segmentación, quieto o PID antiguo
...
def main():
    ...
    # Inicializar CSV de control (cabecera solo si no existe)
    global CSV_CONTROL_PATH
    if not os.path.exists(CSV_CONTROL_PATH):
        with open(CSV_CONTROL_PATH, "w") as f:
            f.write("image,v,w\n")
