...
def lidar_callback(lidar_data, raw_point_cloud, filtered_point_cloud, downsampled_point_cloud, frame, noise_std=0.1, attenuation_coefficient=0.1, output_dir = 'dataset/lidar'):
    """
...
# 📌 Etapa 1: Puntos originales (sin modificaciones)
    # Asignar colores originales antes de procesar los puntos
    raw_colors = np.array([get_color_from_semantic(tag) for tag in semantic_tags]) / 255.0

    # Guardar la nube de puntos original (sin ruido ni pérdidas)
    raw_point_cloud.points = o3d.utility.Vector3dVector(points)
    raw_point_cloud.colors = o3d.utility.Vector3dVector(raw_colors)
...
# 📌 Etapa 2: Puntos después de ruido y pérdidas
    filtered_colors = np.array([get_color_from_semantic(tag) for tag in semantic_tags]) / 255.0
    ...
    points,semantic_tags = voxel_downsampling(points, semantic_tags)
...
def lidar_standard_callback(lidar_data, point_cloud_standard, frame):
    '''
    ...
    data[:, 0] = -data[:, 0]  # Invertir el eje X para corregir la orientación
    ...
    point_cloud_standard.points = o3d.utility.Vector3dVector(data[:, :-1])
    point_cloud_standard.colors = o3d.utility.Vector3dVector(int_color)
...
def spawn_vehicle_lidar_camera_segmentation(world, bp, traffic_manager, delta):
    ...
    # 📌 Configuración del LiDAR SEMÁNTICO
    ...
    # 📌 Configuración del LiDAR ESTÁNDAR (Mismo setup, sin etiquetas semánticas)
    lidar_standard_bp = bp.find('sensor.lidar.ray_cast')
    lidar_standard_bp.set_attribute('channels', '64')
    lidar_standard_bp.set_attribute('range', '100')
    lidar_standard_bp.set_attribute('points_per_second', '1000000')
    lidar_standard_bp.set_attribute('rotation_frequency', str(1 / delta))
    lidar_standard_bp.set_attribute('upper_fov', '30')  # Incrementar el límite superior
    lidar_standard_bp.set_attribute('lower_fov', '-45')  # Reducir el límite inferior
    lidar_standard_bp.set_attribute('noise_stddev', '0.1')
    lidar_position = carla.Transform(carla.Location(x=-0.5, z=1.8))
    lidar_standard = world.spawn_actor(lidar_standard_bp, lidar_position, attach_to=vehicle)
    ...
    return vehicle, lidar, lidar_standard, camera, segmentation_camera
...
def main():
    ...
    vehicle, lidar,lidar_standard, camera, segmentation_camera = spawn_vehicle_lidar_camera_segmentation(world, blueprint_library, traffic_manager, delta)
    actor_list.append(vehicle)
    actor_list.append(lidar)
    actor_list.append(lidar_standard)
    ...
    raw_point_cloud = o3d.geometry.PointCloud()  # Nube original
    filtered_point_cloud = o3d.geometry.PointCloud()  # Nube procesada
    downsampled_point_cloud = o3d.geometry.PointCloud()  # Nube submuestreada
    standard_point_cloud = o3d.geometry.PointCloud()

    frame = 0 # Contador de frames
    lidar.listen(lambda data: lidar_callback(data, raw_point_cloud, filtered_point_cloud, downsampled_point_cloud, frame))
    lidar_standard.listen(lambda data: lidar_standard_callback(data, standard_point_cloud, frame))

    # Utilizar VisualizerWithKeyCallback
 # 📌 Crear dos visualizadores SEPARADOS
    viz_raw = o3d.visualization.Visualizer()         # Puntos originales
    viz_filtered = o3d.visualization.Visualizer()    # Puntos después de ruido y pérdidas
    viz_downsampled = o3d.visualization.Visualizer() # Puntos después del submuestreo
    viz_standard = o3d.visualization.Visualizer()
    
    viz_raw.create_window(window_name="Lidar Original", width=960, height=540, left=100, top=100)
    viz_filtered.create_window(window_name="Lidar Con Ruido Y Pérdidas", width=960, height=540, left=1100, top=100)
    viz_downsampled.create_window(window_name="Lidar Con Submuestreo", width=960, height=540, left=1100, top=100)
    viz_standard.create_window("LiDAR Estándar", width=960, height=540, left=1100, top=100)
    
    for viz in [viz_raw, viz_filtered, viz_downsampled, viz_standard]:
    ...
    def toggle_camera_view(_):
        ...
        set_camera_view(viz_raw, third_person_view)
        set_camera_view(viz_filtered, third_person_view)
        set_camera_view(viz_downsampled, third_person_view)
        set_camera_view(viz_standard, third_person_view)
    ...
    while True:
        ...
        if frame == 5 and not lidar_data_received:
            #viz_raw.add_geometry(raw_point_cloud)  # Nube sin procesar
            viz_filtered.add_geometry(filtered_point_cloud)  # Nube con ruido y pérdidas
            viz_downsampled.add_geometry(downsampled_point_cloud) # Nube con submuestreo
            #viz_standard.add_geometry(standard_point_cloud)
        ...
        #viz_raw.update_geometry(raw_point_cloud)
        viz_filtered.update_geometry(filtered_point_cloud)
        viz_downsampled.update_geometry(downsampled_point_cloud)
        #viz_standard.update_geometry(standard_point_cloud)

        #viz_raw.poll_events()
        viz_filtered.poll_events()
        viz_downsampled.poll_events()
        #viz_standard.poll_events()

        #viz_raw.update_renderer()
        viz_filtered.update_renderer()
        viz_downsampled.update_renderer()
        #viz_standard.update_renderer()
        ...
        if not viz_filtered.poll_events() or not viz_raw.poll_events() or not viz_downsampled.poll_events() or not viz_standard.poll_events():
